...
const NUM_FIELDS = ['calories', 'proteinG', 'carbsG', 'fatG'];
...
async function list(req, res, next) {
  try {
    ...
    const rows = await NutritionLog.findAll({ where, order: [['date', 'ASC']] });
    res.json(rows);
  } catch (err) { next(err); }
}
...
async function create(req, res, next) {
  try {
    ...
    if (Object.keys(data).length === 0) {
      return res.status(400).json({ error: { message: 'at least one nutrition field is required' } });
    }
    const [row, created] = await NutritionLog.findOrCreate({
...
async function update(req, res, next) {
  try {
    const row = await NutritionLog.findOne({
      where: { id: req.params.id, userId: req.user.uid },
    });
    if (!row) return res.status(404).json({ error: { message: 'Nutrition log not found' } });
    const data = pickFields(req.body);
