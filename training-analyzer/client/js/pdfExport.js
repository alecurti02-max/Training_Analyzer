  ...
  const radarUrl = await chartToImage({
    ...
    data: {
      ...
      datasets: [{
        label: 'Profilo',
        data: values,
        backgroundColor: 'rgba(224,32,32,0.20)',
    ...
    options: {
      ...
      scales: { r: { min: 0, max: 100, ticks: { stepSize: 25, color: '#888', font: { size: 11 } }, pointLabels: { font: { size: 13 } } } },
    },
...
async function renderTrendChartsSection(doc, ctx) {
  ...
  const charts = [
    {
      title: 'Peso (kg)',
      data: { labels: weights.map((w) => w.date.slice(5)), datasets: [{ label: 'kg', data: weights.map((w) => w.value), borderColor: COLORS.red, backgroundColor: 'rgba(224,32,32,0.15)', tension: 0.3, fill: true, pointRadius: 2 }] },
      empty: !weights.length,
    },
    {
      title: 'Tonnellaggio settimanale (kg)',
      data: { labels: tonnageWeekly.map((b) => b.date.slice(5)), datasets: [{ label: 'kg', data: tonnageWeekly.map((b) => Math.round(b.value)), borderColor: COLORS.blue, backgroundColor: 'rgba(41,128,185,0.15)', tension: 0.3, fill: true, pointRadius: 2 }] },
      empty: !tonnageWeekly.length,
    },
    {
      title: 'Km running settimanali',
      data: { labels: kmWeekly.map((b) => b.date.slice(5)), datasets: [{ label: 'km', data: kmWeekly.map((b) => +b.value.toFixed(1)), borderColor: COLORS.green, backgroundColor: 'rgba(39,174,96,0.15)', tension: 0.3, fill: true, pointRadius: 2 }] },
      empty: !kmWeekly.length,
    },
    {
      title: 'Massa grassa (%)',
      data: { labels: bf.map((m) => m.date.slice(5)), datasets: [{ label: '%', data: bf.map((m) => m.bodyFat), borderColor: COLORS.orange, backgroundColor: 'rgba(230,126,34,0.15)', tension: 0.3, fill: true, pointRadius: 2 }] },
      empty: !bf.length,
    },
  ];
